...
def normalize_angle_tensor(x, idx):
    ...
    """
    if len(x.shape) == 2:
        x_norm = torch.zeros(x.shape[0], x.shape[1]+1).to(x.device)
        x_norm[:, :idx] = x[:, :idx]
        x_norm[:, idx] = torch.cos(x[:, idx])
        x_norm[:, idx+1] = torch.sin(x[:, idx])
        x_norm[:, idx+2:] = x[:, idx+1:]
        return x_norm
    elif len(x.shape) == 3:
        x_norm = torch.zeros(x.shape[0], x.shape[1], x.shape[2]+1).to(x.device)
        x_norm[:, :, :idx] = x[:, :, :idx]
        x_norm[:, :, idx] = torch.cos(x[:, :, idx])
        x_norm[:, :, idx+1] = torch.sin(x[:, :, idx])
        x_norm[:, :, idx+2:] = x[:, :, idx+1:]
...
def fold_angle_tensor(x, idx):
    if len(x.shape) == 2:
        x_fold = torch.zeros(x.shape[0], x.shape[1]-1).to(x.device)
        x_fold[:, :idx] = x[:, :idx]
        x_fold[:, idx] = torch.atan2(x[:, idx+1], x[:, idx])
        x_fold[:, idx+1:] = x[:, idx+2:]
        return x_fold
    elif len(x.shape) == 3:
        x_fold = torch.zeros(x.shape[0], x.shape[1], x.shape[2]-1).to(x.device)
        x_fold[:, :, :idx] = x[:, :, :idx]
        x_fold[:, :, idx] = torch.atan2(x[:, :, idx+1], x[:, :, idx])
        x_fold[:, :, idx+1:] = x[:, :, idx+2:]
        return x_fold