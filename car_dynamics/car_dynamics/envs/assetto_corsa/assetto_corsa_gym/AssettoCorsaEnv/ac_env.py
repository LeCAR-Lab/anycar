...
class AssettoCorsaEnv(Env, gym_utils.EzPickle):
    ...
    def set_actions(self, actions):
        ...
        # print("setting actions...", self.actions)
    ...
    def step(self, actions=None):
        ...
        """
        #actions = [throttle, steer] Range: -1 -> 1

        acc_actions = np.zeros(3)
        acc_actions[0] = -1*actions[1]
        if actions[0] >= 0: #accelerating
            acc_actions[1] = (actions[0] * 2) - 1
            acc_actions[2] = -1
        else: # braking
            acc_actions[1] = -1
            acc_actions[2] = (actions[0] * -2) - 1
        self.ep_steps += 1
        self.total_steps += 1

        actions = acc_actions # convert into env format
    ...
    def reset(self, seed=None):
        ...
        if self.n_episodes == 1:
            ...
            for i in self.static_info:
                ...
                if i == "CAR_WHEELBASE":
                    self.wheelbase = self.static_info[i]
                elif i == "CarName":
                    self.car_name = self.static_info[i]
        ...
        self.start_actions = np.array( [0.0, 0.0] )
    ...
    @property
    def pose(self):
        x = self.state["world_position_x"]
        y = self.state["world_position_y"]
        z = self.state["world_position_z"]
    ...
    @property
    def lin_vel(self):
        xvel = self.state["local_velocity_x"]
        yvel = self.state["local_velocity_y"]
        zvel = self.state["local_velocity_z"]
        return np.array([xvel, yvel, zvel])
    ...
    def obs_state(self):
        ...
        vy = self.lin_vel[1]
        yaw = self.rpy[2] 
        wz = self.state["angular_velocity_z"]
